...
class TypeAnalyser:
    ...
    def _infer_from_facts(self, facts: FrozenSet[Fact]):
        def infer_from_fact(fact: Fact):
            ...
            if all(t != UNK_TYPE for t in self.declarations[head.name]):
                if not check_equality(decl_types, self.declarations[head.name]):
                    raise TypeError(
                        f"Declaration of relation {head.name} is inconsistent with the"
                        f" new fact {fact}."
                        f" Declaration: {self.declarations[head.name]}. Inferred decl types: {decl_types}."
                    )
            else:
    ...
    def _infer_from_rules(self, rules: FrozenSet[Rule]):
        ...
        while is_changed:
            ...
            for rule in rules:
                # debug
                if rule.head.name == "data_reach":
                    print("")
                if rule.head.name == "control_flow":
                    print("")
                for idx, head_arg in enumerate(rule.head.args):
                    ...
                    inferred_types = {
                        ...
                        if isinstance(literal, Literal) and head_arg in literal.args
                    }
                    # Find inferred types from body unifications
                    if collect(rule.body, lambda x: isinstance(x, Unification)):
                        logger.error("Infer unification types not supported yet.")
