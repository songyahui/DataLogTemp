...
class ProgramBuilder:
    ...
    @staticmethod
    def get_edb_idb_names(
        rules: FrozenSet[SouffleRule],
    ) -> Tuple[Set[str], Set[str]]:
        """Get the names of the input and output relations from the given rules.

        :param rules: The rules of the program
        :type rules: FrozenSet[souffle.Rule]
        :return: The names of the input and output relations
        :rtype: Tuple[Set[str], Set[str]]
        """
        rule_head_names = set(map(lambda x: x.head.name, rules))
        rule_body_names = set(
            [l.name for r in rules for l in r.body if isinstance(l, SouffleLiteral)]
        )
        edb_names = rule_body_names - rule_head_names
        idb_names = rule_head_names
        return edb_names, idb_names
    ...
    def infer_whole_program(
        ...
        edb_names, idb_names = ProgramBuilder.get_edb_idb_names(rules)
