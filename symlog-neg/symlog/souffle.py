...
Unification = namedtuple_with_methods(
    namedtuple("Unification", ["lhs", "rhs", "equal"])
)
...
class ASTConstructor(Transformer):
    ...
    def rule(self, head, body):
        self.program.rules.add(Rule(head, body))

    def fact(self, head):
        self.program.facts.add(Fact(head, [], False))
...
def pprint(node):
    ...
    def pprint_unification(u):
        if u.equal:
            return f"{pprint_term(u.lhs)} = {pprint_term(u.rhs)}"
        else:
            return f"{pprint_term(u.lhs)} != {pprint_term(u.rhs)}"
        ...
        if rule.body:
            ...
            if rule.body:
                for el in rule.body:
                    ...
                    elif isinstance(el, Unification):
...
def transform(node, f):
    ...
    def transform_unification(u, f):
        return f(
            Unification(transform_term(u.lhs, f), transform_term(u.rhs, f), u.equal)
        )
...
def collect(node, p):
    ...
    def collect_unification(u, p):
        collect_term(u.lhs, p)
        collect_term(u.rhs, p)
        if p(u):
            result.append(u)
